...
@jit
def _predictions_core(beta: jnp.ndarray, X: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray]:
    pred = X @ beta
    jac = X
    return pred, jac


def predictions(beta: jnp.ndarray, X: jnp.ndarray, vcov: jnp.ndarray
                ) -> dict[str, np.ndarray]:
...
@jit
def _predictions_byT_core(beta: jnp.ndarray, X: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray]:
    pred = jnp.mean(X @ beta)
    jac = jnp.mean(X, axis=0)
    return pred, jac


def predictions_byT(beta: jnp.ndarray, X: jnp.ndarray, vcov: jnp.ndarray
                    ) -> dict[str, np.ndarray]:
